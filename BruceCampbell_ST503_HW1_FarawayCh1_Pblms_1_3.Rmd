...
When loading and inspecting the data we will note which variables are numberic, and which are strings, we'll also be on the lookout for variables that we may want to encode as factors.  Here we note that gender is a candidate for such encoding.
...
We note a relationship between lcavol and lpsa, and between lcp and lpsa.  We also note that there is a replationship between the gleason score and lpsa and pgg45. 
